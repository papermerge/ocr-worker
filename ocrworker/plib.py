...
def base_thumbnail_path(uuid: UUID | str) -> Path:
    ...
    return Path(
        const.THUMBNAILS, const.JPG, uuid_str[0:2], uuid_str[2:4], uuid_str
    )


def thumbnail_path(
    uuid: UUID | str, size: int = const.DEFAULT_THUMBNAIL_SIZE
) -> Path:
    """
    Relative path to the page thumbnail image.
    """
...
def abs_thumbnail_path(
    ...
    return Path(
        settings.papermerge__main__media_root, thumbnail_path(uuid, size)
    )


def docver_base_path(uuid: UUID | str) -> Path:
    uuid_str = str(uuid)

    return Path(const.DOCVERS, uuid_str[0:2], uuid_str[2:4], uuid_str)


def docver_path(uuid: UUID | str, file_name: str) -> Path:
    uuid_str = str(uuid)

    return docver_base_path(uuid_str) / file_name


def abs_docver_path(uuid: UUID | str, file_name: str):
    return Path(
        settings.papermerge__main__media_root, docver_path(uuid, file_name)
    )
...
def page_path(
    ...
    return Path(const.OCR, const.PAGES, uuid_str[0:2], uuid_str[2:4], uuid_str)
